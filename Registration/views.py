...
# from General.models import Shift, StudentDetail, CollegeYear, BranchSubject, Semester, \
#     FacultySubject, Batch, StudentSubject, YearBranch
...
#     year_branch = YearBranch.objects.filter(branch=branch)
...
#             division = DivisionDetail.objects.get(pk=form.cleaned_data.get('division'))
#
...
#             division_obj = DivisionDetail.objects.get(branch=branch_obj, year=year_obj, division=division, shift__shift=shift)
...
#
#             # linking with the subject -------------------------------------------------------------------------
#             year_branch = YearBranch.objects.filter(branch=branch, year=year)
#
#             branch_subject = BranchSubject.objects.filter(year_branch=year_branch[0])
#             student_subject_list = []
#             for each_branch_subject in branch_subject:
#                 student_subject_list += [
#                     StudentSubject(student=student, subject=each_branch_subject.subject, is_active=True)]
#
#             StudentSubject.objects.bulk_create(student_subject_list)
#
#             # ends linking with subject -----------------------------------------------------------------------
#
...
#     division_list = DivisionDetail.objects.filter(year_branch__branch=Branch.objects.get(branch=branch)).values_list('division',
#                                                                                                             flat=True)
...
#     division_list = DivisionDetail.objects.filter(shift=Shift.objects.get(shift=shift),
#                                                       year_branch__branch=Branch.objects.get(branch=branch)).values_list('division',
#                                                                                                             flat=True)
#     return HttpResponse(division_list)
#
#
...
#             year_branch = YearBranch.objects.filter(branch=branch_object, year=year_obj)
#             branch_subject = BranchSubject(year_branch=year_branch[0],
